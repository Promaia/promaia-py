...
class ResultValidator:
    ...
    def validate_results(intent: Dict[str, Any], results: List[Dict[str, Any]], query_mode: str = "sql") -> Tuple[bool, str]:
        ...
        # Normalize database names for comparison (handle both "stories" and "workspace.stories" formats)
        def normalize_db_name(db_name: str) -> str:
            """Extract the nickname from qualified names like 'workspace.stories' -> 'stories'"""
            if db_name is None:
                return ''
            if '.' in db_name:
...
def _get_content_display_text(result: Dict[str, Any], db_path: str = None) -> str:
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            elif content_type == 'notion' or database_name in ['stories', 'yp', 'notion', 'projects', 'epics', 'journal', 'awakenings']:
