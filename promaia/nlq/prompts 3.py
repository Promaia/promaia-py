    ...
    try:
        ...
        if not workspace:  # Only show overview when not filtering by workspace
            ...
            if workspace_map:
                ...
                preview += "\n**IMPORTANT**: When querying, always specify the workspace (e.g., 'acme.stories') or use the workspace parameter.\n\n"
...
<tool_call>
  ...
  <parameters>
    <query>acme gmail from eddie last 7 days</query>
...
- ALWAYS specify the workspace when querying (e.g., workspace="acme" parameter OR "acme gmail" in query text)
- Check the "Workspace Overview" in the data sources section to see which databases belong to which workspaces
- If user mentions a workspace name, search that workspace's databases
- Example: User asks about "acme stories" → Use workspace="acme" or include "acme stories" in query text
...
**When to use**:
- Searching for SPECIFIC, CONCRETE keywords that appear in content (names like "Alice", "Product X", specific phrases)
- Finding messages FROM a specific person by name (e.g., "emails from Alice")
...
**Parameters**:
- `query`* (string): Natural language description specifying workspace, database, and search terms that appear in content. Format: "{workspace} {database} from/about {text_keywords} {time_filter}". Examples: "acme gmail from alice about launch last 30 days", "acme journal mentioning meeting last month"
...
<tool_call>
  <tool_name>query_sql</tool_name>
  <parameters>
    <query>acme gmail from alice about launch last 30 days</query>
    <reasoning>User asked "What did Alice say about the product launch?" My current context doesn't contain any emails from Alice. I'm querying the gmail database because Alice communicates via email. I'm searching for messages where Alice is the sender AND the content mentions "product launch" or "launch" from the last 30 days. I expect to find 5-15 recent emails with her feedback, concerns, and updates about the launch timeline and strategy.</reasoning>
...
- "emails from alice" → query_vector can filter by sender property
...
AI: I need to search for emails from Alice about the product launch.

<tool_call>
  <tool_name>query_sql</tool_name>
  <parameters>
    <query>emails from Alice about product launch</query>
    <reasoning>User asked what Alice said about the product launch. My current context doesn't contain any emails from Alice. I'm querying the gmail database because that's where Alice's communications would be stored. I'm searching for messages where Alice is the sender and the content mentions "product launch" or "launch". I expect to find several emails with his feedback, questions, or updates about the launch.</reasoning>
  </parameters>
</tool_call>

[User sees reasoning, approves, context is loaded]

AI: Based on the emails I found, Alice mentioned...
```
